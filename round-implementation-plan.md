...
**Local database: PostgreSQL in Docker — confirmed.** Run the same PostgreSQL major version Neon uses (16) via a `docker-compose.yml`, so schema and query behaviour are identical across environments. One command (`docker compose up`) brings up the full local dependency set (Postgres).
...
**Grounded Completions: the endpoint contract, shipped in Step 32 and kept as the brief's named RAG capability.** (The steps that consume the corpus use the Search path in the next decision; this is the contract of the one-call endpoint itself.) Grounded Completions is its own endpoint — `POST https://platform.ai.gloo.com/ai/v2/chat/completions/grounded`, not reachable through the OpenAI SDK (our client already uses plain `fetch`, so this costs nothing) — taking the standard Completions params plus `rag_publisher` (names our corpus, case-sensitive), `sources_limit` (1–10, default 3), `include_citations` (**defaults to `false`**, so it must be passed explicitly), `tradition` (`evangelical` / `catholic` / `mainline` / `not_faith_specific`), and exactly one routing choice (`auto_routing` / `model_family` / `model`). Retrieval and generation happen in a single call; the response returns a `citations` array (`item_title`, `item_url`, `author`, `publisher`, `publication_date`, `snippets`) plus a `sources_returned` boolean. In the streaming variant citations arrive as the first SSE chunk — not needed by any step here, which uses the non-streamed call.

**Retrieval architecture: Search + Completions V2 with a filename prefilter, not one-call Grounded Completions — measured, not assumed.** Grounded Completions ships and stays (Step 32, with its `/api/dev/grounded` route as the demonstrated capability), but the *consuming* steps use Gloo's Search API (`POST /ai/v1/data/search`, Pro plan; body `collection: "GlooProd"`, `tenant`, `query`, `limit`, `certainty`) followed by an ordinary Completions V2 call — the pattern Gloo's own docs recommend when you need "full control over context, prompts, ordering". The `tenant` is the **normalized** publisher name, underscores not hyphens (`scripture_commentary`, not `scripture-commentary` — the hyphen form returns 403), and is configured explicitly as `GLOO_SEARCH_TENANT` rather than derived, since the normalization rules are undocumented. Search returns chunk-level hits carrying `filename`, `item_title`, `part` (chunk index), `snippet` and `certainty`; `producer_id` comes back `null`, so **`filename` is the filter key**. Server-side filtering does not exist (`producer_id`, `item_title`, `filters`, `where` in the body are silently ignored), so the caller over-fetches (`limit: 100`, `certainty: 0`) and filters client-side to the one file the passage maps to. Measured against the live corpus, that beats Grounded Completions on every axis that matters here: exact chapter scoping instead of semantic drift (Grounded returns Psalm 93 and Psalm 131 citations for Psalm 23 and 121 queries); a **deterministic** coverage signal (uncovered Psalm 119 yields exactly 0 on-target chunks, where `sources_returned` unreliably reported `true` with Psalms 106/118/128/149 citations); ~10× the grounding volume (11,636 chars of the Psalm 23 item — 74% of it — versus 1,075 at `sources_limit: 3` and 2,474 at 10); and `part` restores reading order before prompting. Trade-off accepted knowingly: brief §3.2 names "Grounded Completions (RAG)" as the capability, so that endpoint stays shipped, working and demonstrable, and the writeup states plainly why production retrieval uses the Search path.

**The corpus is pre-split per Psalm because Gloo's chunking is forced and per-item identity is the only scope signal.** Ingestion always chunks — "there is no setting to disable it, bypass it, or tune its size and overlap" — into roughly paragraph-sized chunks with deliberate overlap between consecutive chunks. The chunk is the retrieval unit, and every chunk carries its parent item's `filename` and `item_title` — so one item per book+chapter is what turns those fields into an exact scope key. A single monolithic upload would label every chunk `psalms-commentary.md` / "Psalms Commentary", leaving nothing to distinguish Psalm 22 from Psalm 23 and no way to detect coverage at all, and its overlapping chunks would straddle Psalm boundaries. Gloo's own guidance is the remedy: "if your content must not be merged with a neighbor… upload each unit as its own item." Hence one item per book+chapter (per Psalm for the Psalms corpus), a strict `psalm-NNN.md` filename, a strict `item_title` convention ("<Source> — <Book> <Chapter>", e.g. "Treasury of David — Psalm 23"), and the same reference as a heading in the item's own text. **The filename is the scope mechanism, not a hint:** a passage maps to exactly one file, chunks from any other file are discarded before anything is prompted, and zero surviving chunks means "not covered" with no inference required. Search queries still name the passage in **human form** ("Psalm 23 commentary"), never USFM — retrieval is semantic over chunk text and `PSA.23` matches nothing — but a mis-phrased query can now only cost recall, never correctness, because the filter runs on identity rather than on similarity. Corpus growth beyond the Psalms PDF (public-domain whole-Bible commentaries, pre-split the same way, same filename convention) requires **zero code change** — same tenant, same filter — and is scheduled as optional data ops after Step 33 ships, with the Psalms-only corpus always remaining a sufficient fallback.

**Prayer requests: private by default, per-request sharing, anonymous prayed-count, author-owned "answered".** Prayer requests are a first-class object (`prayer_requests`: author, text, status `active`/`answered`/`archived`, visibility `private`/`circle`; `prayer_acts`: request, member, prayed_at — one per member per request). A request is private by default and is shared to the circle only by an explicit per-request action, mirroring the highlight-sharing rule (explicit per-item consent, never automatic). Shared requests render as a distinct card in the circle thread and pass through the Translation Agent like any member message. Members respond with a one-tap **"I prayed"**; the author sees only an anonymous count ("3 people prayed for this") — never names — one tap per member per request, consistent with the no-comparison constraint (no "who hasn't prayed" is ever derivable from the UI). Only the author can mark a request **answered** — answering is testimony, not task completion; members' prayer acts never change a request's status. Every request text passes through the Escalation Agent before any other processing, same gate as reflections — prayer requests are a high-likelihood surface for crisis language. Answered requests build a private answered-prayer timeline on the author's profile. Instant Access users can pray for others' shared requests; their own requests live for the session only (no persistence), consistent with Path B rules. AI hooks (answered-prayer detection, digest celebration) are specified in Steps 35A/35B. (Note: prayer *requests* here are a shared circle object, distinct from the Step 26 Prayer tab's private personal prayer — the two are not conflated.)
...
### Step 7A — Copyright attribution on imported highlight snippets (retrofit)
- **What gets built:** The profile's imported-highlight sample entries display Bible text snippets, so per the global attribution constraint each row must show the copyright attribution of the version the highlight was made in, keyed to the version ID already stored with the highlight (Step 7 schema — no migration needed). Attribution text is resolved from the version catalogue; a small reusable attribution component is introduced so Step 13's passage view can reuse it.
- **UI:** A single muted attribution line per highlight row (or grouped once per version if consecutive rows share a version), styled from the existing token scale — unobtrusive, always visible, never truncated away.
- **Why this step comes here:** Step 7 shipped without attribution; this is the minimal retrofit and lands the shared attribution component before Step 13 (passage view) and Step 14 (in-passage highlights) need it.
- **Touches:** Profile highlight list UI, version catalogue lookup, new attribution component.
- **How to test it:** Open the profile of a user with imported highlights from more than one version: each sample entry shows the attribution matching its own stored version ID. A user with no highlights sees the unchanged empty state.
- **Definition of done:** Every displayed highlight snippet carries the correct version copyright attribution, and the attribution component is reusable by later passage-rendering steps.
...
- Every **user-facing agent wait** uses the shared AgentThinking component (born in Step 12): editorial shimmer + per-agent rotating status lines that honestly describe what the agent is doing, personalised from real inputs where possible. Cron-driven agents (Facilitator, Summary, Reminder, Health) have no waiting user and no wait UI; the Escalation Agent is never decorated — it runs silently. Prayer (Step 26) additionally streams its output.
...
- **What gets built:** The reading plan schema: a plan is a structured list of day-by-day passage references (no Bible text ever stored — references only). A seed migration inserts the starter library (Psalms in 30 days, Gospel of Mark, Ruth). Onboarding's plan step lets the user browse and select a plan (once Step 12 lands, the library becomes the secondary path — the AI-generated plan is the recommended default); a plan view shows the day list with the user's current day and per-day references. User plan progress (current day, days completed) is stored privately — never exposed to other members, per the no-comparison constraint.
...
- **What gets built:** The "create my own plan" onboarding path — presented as the **recommended default** at the end of onboarding: after the Step 10 questions, the user lands on a plan screen pre-filled from their answers with "Build my plan" as the single primary action and "browse ready-made plans instead" (the Step 11 library) as a quiet secondary link. PlanBuilder sends onboarding goals, time per day, and topics to Gloo and receives a structured day-by-day plan. **Every reference is validated by a live YouVersion Passages fetch before saving.** Failed references trigger up to 2 single-day regenerations (with the failure fed back to Gloo), then substitution from a curated topic-tagged fallback pool with the day marked "adjusted" in the preview. Generated plans are saved in the identical structure as seeded plans. Total generation failure offers the pre-defined library instead.
- **UI:** Goal input as a friendly library form, pre-filled from the Step 10 profile answers and including the **plan duration** question (choice chips: 1 week / 2 weeks / 30 days) — duration is asked here, not in onboarding, because it is per-plan rather than per-profile; generation shows a warm, branded "thinking" state ("Building your plan…" with skeleton day rows and rotating progress lines referencing the user's own goals, e.g. "Finding passages on forgiveness…"), never a spinner on a blank page. This thinking state is built as a reusable **AgentThinking library component** (editorial-styled shimmer/skeleton + per-agent rotating status lines fed as props, with a light variant for short waits) — the standard wait treatment every later user-facing agent reuses (Steps 15, 21, 26, 35). During the validation phase the rotating lines give way to real progress reporting ("Checking day 4 of 14…") driven by the actual per-reference validation loop. The preview reuses Step 11's day-list component with "adjusted" days marked by a badge and a one-line explanation. Wholesale failure renders the empty-state component with a clear CTA into the pre-defined library. All screens in the Step 8C register (responsive shell, serif headings, one primary action), matching the Step 11 plan screens exactly.
...
### Step 12A — Multiple plans per user: active-plan pointer and plan history
- **What gets built:** The single-plan schema shortcut is removed before anything else writes progress. `user_plan_progress` moves from `user_id` as primary key to a composite `(user_id, plan_id)` key with an explicit active-plan pointer (an `is_active` flag with a partial unique index, or equivalently `users.active_plan_id`). Exactly one plan is active per user at a time — the doc's single-spine model is unchanged — but selecting or generating a new plan now **pauses** the previous row instead of overwriting it, preserving `completed_days`. `loadUserPlanState` resolves the active row; a new query lists the user's paused plans. Switching back to a paused plan resumes it exactly where it left off. AI-generated plans remain personal (never in the shared library), but stay visible to their owner here after being replaced. Anonymous sessions are unaffected (token carries one plan; no history without persistence, per the brief).
- **UI:** The `/plan` screen keeps the active plan as its hero (current day, "Continue reading" — unchanged from Step 11). Below it, a "Your plans" section lists paused plans as flat cards (name, length, progress as *n of m days* — private, own-eyes only) with a "Resume" action behind a confirmation ("Switch your reading to …?"). The pre-defined library remains browse-only ("start a new plan") and is never mixed into "Your plans." Empty state: section hidden entirely when there are no paused plans.
- **Why this step comes here:** Step 14 ("Finished reading") is the first heavy writer of `completed_days`; landing the composite key now means Step 14 targets the final shape and no data migration of real progress is ever needed. Also required by Step 16: joining a circle switches the active plan to the circle's plan, which must pause — not destroy — the personal plan. Depends on Steps 11–12.
- **Touches:** Database migration (`user_plan_progress` PK change, active pointer), `src/lib/plans.ts` (active-row resolution, paused-plans query), `/plan` screen UI.
- **How to test it:** Generate an AI plan, complete Day 1, then select "Psalms in 30 days" from the library: the Psalms plan becomes active, and the AI plan appears under "Your plans" showing 1 of 14 days. Resume the AI plan: Day 2 is current — progress survived the round-trip. Confirm one and only one active row exists per user in the database, and that no other user can see any of it.
- **Definition of done:** A user always has exactly one active plan, every previously started plan is listed and resumable with progress intact, and nothing about any plan's progress is visible to anyone else.

## Phase 4 — The Reading Experience

### Step 13 — Passage view
- **What gets built:** The core reading screen for "today's passage": text fetched live from the YouVersion Passages API using the session's version ID; the version abbreviation shown persistently next to the reference; a one-tap version switcher (fed by Step 9's catalogue); the fallback behaviour — if the passage is unavailable in the chosen version, fetch the language-default version and show a notice without overwriting the user's preference; and an "Open in Bible App" deep-link button on every passage view. Wherever Bible text is displayed, the Bible version's copyright attribution is always shown alongside it, in accordance with the license agreement — including after a version switch or a fallback fetch, where the attribution shown must match the version actually rendered.
- **UI:** The reading screen is the app's flagship surface — designed for comfortable long-form reading: generous line height and reading-size type from the scale, sticky reference header, version abbreviation as a tappable chip opening a bottom-sheet version switcher (bottom sheet added to the library), "Open in Bible App" as a secondary button, fallback notice via the info banner, a text skeleton while the passage loads, and the version's copyright attribution rendered unobtrusively but always visible at the end of the passage text.
...
- **How to test it:** As a Spanish/NVI user, open Day 1: the passage renders in Spanish with "NVI" beside the reference. Tap the version badge, switch to RVES: text re-fetches in one tap. Tap "Open in Bible App": the deep link opens the correct passage in YouVersion. Point a test user at a version lacking the passage: the language-default version renders with the fallback notice. In every case (initial load, version switch, fallback), the copyright attribution shown below the passage matches the version actually displayed.
- **Definition of done:** Any user on either path reads today's live-fetched passage in their chosen version, can switch versions in one tap, can deep-link to the Bible App, and always sees the correct version copyright attribution with the text.

### Step 14 — In-app highlighting and reading completion
- **What gets built:** Text selection in the passage view creates a session highlight, stored with the reference and **the version ID it was made in** (per the brief). A "Finished reading" action marks the plan day complete and advances private progress — written to the user's **active** `user_plan_progress` row (Step 12A shape). Signed-in users' highlights persist; anonymous users' highlights live in session storage only. No progress or highlight information is ever visible to other members.
...
- **UI:** Prompts live in a collapsible card (library card + disclosure header) above the passage, visually marked as personal ("Just for you" treatment), with the AgentThinking component (light variant, e.g. "Preparing your prompts…") while generating and a silent hidden state on failure — the reading experience never blocks on this card.
...
- **What gets built:** Circle entity with states (forming → active → stalled → archived), size limits (min 2, max 5), and an attached reading plan. Users can create a circle (choosing a plan), browse open circles (name, plan, member count — never member progress), and join one. A circle becomes `active` at 2 members; joining is blocked at 5. Members list shows display names only. **Joining a circle aligns the member's reading with the circle's attached plan:** if the joiner's active plan differs, an explicit prompt ("This circle reads the Gospel of Mark — switch your reading?") makes the circle's plan their active plan and pauses the personal plan (Step 12A — progress preserved, resumable later). Never a silent swap; declining cancels the join. Everything downstream (digests, starters, reminders) assumes circle members share the circle's plan.
...
- **How to test it:** With test accounts: account A creates a circle on the Mark plan (state `forming`); account B browses, finds it, joins; on B's join the state flips to `active`. Add three more members (reaching 5), then a sixth attempt is refused with a clear message. Confirm the browse view exposes no per-member progress anywhere. Join with an account whose active plan is Psalms: the switch prompt appears, accepting makes Mark active and lists Psalms as paused under "Your plans"; declining leaves the user out of the circle with their plan untouched.
...
- **What gets built:** The daily reflection flow: at the end of the day's reading, the member is prompted to write a reflection **in the circle thread itself** — not on a separate screen. The reflection is a distinct *submission type* underneath (not an ordinary chat message): it carries the plan-day tag and follows its own submit path. That path is hard-wired so the **Escalation Agent runs first, synchronously, before any other processing** — before the reflection is posted to the thread, before translation, before anything. This gated, day-tagged submit is what separates a reflection from a plain thread message (ordinary messages do not pass through the synchronous Escalation gate). Unflagged reflections post to the thread as a distinct "reflection" message type tied to the plan day. Flagged reflections are saved privately to the author (never posted, excluded from future digest input, per the Decisions section), and the author sees the support card.
- **Reflecting more than once on a day is allowed, deliberately.** A reader who reopens a passage later may finally have words for it, and the self-paced/no-comparison constraints mean the app never tells someone they have already had their turn. There is therefore no unique constraint on (author, day); the UI instead names the existing reflection ("Add another reflection" on the reading screen, and a composer line saying this adds to what they already shared) so a second one is chosen rather than stumbled into. Consumers must count **distinct authors**, never rows — see Step 24.
- **UI:** No separate reflection screen. Finishing the day's reading (Step 14's completion action) drops the member into the circle thread with the **same composer primed for reflection mode**: today's reference shown above it and a prompt ("Share your reflection on today's passage") replacing the ordinary placeholder, so it's unmistakably a reflection and not a throwaway line. Submitting shows a pending state (the synchronous Escalation gate means this send behaves differently from a plain message — it may hold briefly, and a flagged one is redirected to private rather than posted). Reflections render in the thread as visually distinct reflection cards tagged with the plan day, clearly different from ordinary messages. The composer returns to ordinary chat mode once the reflection is submitted.
...
- **Touches:** Thread composer (reflection mode), submission pipeline, Escalation agent, database (`reflections`, messages), thread UI.
- **How to test it:** Finish the day's reading: the thread opens with the composer primed for reflection (reference + reflection prompt shown). Submit a normal reflection: it appears in the circle thread tagged to today's passage, the composer reverts to ordinary chat, and `agent_logs` shows the Escalation run *preceding* the post. Submit a reflection containing the crisis test phrasing: the support card appears to the author; a second member's view of the thread shows **no trace** of that reflection or any escalation signal.
...
### Step 19A — Private per-passage notes
- **What gets built:** A private, free-text note a reader can keep against the passage they are reading — distinct from both highlights (Step 14, a selected phrase) and reflections (Step 19, circle-facing) — plus a **centralised "My notes" list** on the Profile screen so a reader can see every note in one place, not only by returning to each passage. One editable note per (user, passage reference), saved as the user types (debounced autosave) and never posted, shared, or fed to any agent. Keyed by the plan day's **passage reference**, not the version ID (a note is about the passage, not the exact rendered translation — unlike a highlight, which carries its version). Signed-in users' notes persist in a new `notes` table; anonymous users' notes live in session storage only, exactly like in-app highlights (no database row — brief §7). **This is an app-managed feature, not the YouVersion Notes API** (brief §85 forbids implementing that API; app-managed private notes, like in-app highlights and reflections, are unaffected — no Scripture text is stored, only the reference the note hangs on).
- **UI:** Two surfaces over the same data. **(1)** On the reading screen, a quiet note surface in the editorial register, clearly marked private ("Only you can see this"), sitting near the highlights affordance below the passage: a collapsible card with a single autosizing textarea, an unobtrusive "Saved" indicator after a write settles, and a gentle empty state ("Jot a thought for yourself"). No send button — it is a working scratch surface, not a post. **(2)** On Profile, a "My notes" section (Home principle §63 — private collections live in Profile, alongside the Step 35B answered-prayer timeline): notes reverse-chronological by last-edited, each row showing the passage label, a note preview, and a relative last-edited time; tapping a row opens that passage's reading screen with the note in focus for editing. Empty state when the reader has no notes yet. Both surfaces never block reading and hide silently on a load/save failure (the text stays in the box to retry).
- **Why this step comes here:** A reading-experience feature and a natural sibling of highlighting; placed as 19A (not renumbered into Phase 4) to avoid disturbing the numbering of every later step. Depends on the passage view (Step 13), the private-content patterns from Step 14 (per-reference storage, Path B session storage), and the Profile screen (Step 8C). Independent of the circle — it works with or without a circle and is never part of any circle-facing view.
- **Touches:** Passage UI, Profile UI, database (`notes`), anonymous session state.
- **How to test it:** As a signed-in user, open today's passage, type a note, and navigate away and back: the note is still there. Switch the passage to another version: the same note shows (it hangs on the reference, not the version). Add notes on two more passages, then open Profile: "My notes" lists all three, most-recently-edited first, each with its passage label and preview; tap one and land on that passage with the note in focus. Confirm no other member can see any of it and that notes appear in no thread, digest, or agent input. As an anonymous user, a note persists within the session (session storage) and the "My notes" list reflects the session's notes only — both gone in a fresh session, with no database row ever written for Path B.
- **Definition of done:** A reader can keep one private, autosaved free-text note per passage on both session paths, tied to the passage reference and visible to no one else; every note is reachable both from its passage and from a single "My notes" list on Profile; and nothing about any note is exposed to any circle or agent.

### Step 20 — PostReading Agent: conversation starters in the thread
- **What gets built:** When the **first member of a circle** finishes a day's reading (Step 14's completion action), the PostReading agent generates 2–3 discussion questions via Gloo, grounded in the specific passage, that member's session highlights, and the pre-reading prompts they were shown. Posted to the circle thread attributed to **"Round"** as a system message type (visually distinct from member messages); members can reply to them. **Idempotent per circle+plan day — one card per circle per day, not one per reader:** later finishers add nothing, so a five-member circle reading the same day sees one starter card rather than five near-identical ones. The card lands early, so it is waiting in the thread when the rest of the circle arrives.
...
- **How to test it:** Highlight a striking phrase, tap "Finished reading": within moments 2–3 questions attributed to "Round" appear in the circle thread, at least one visibly connected to the highlighted phrase. Another member replies to one. Complete the same day again — no duplicate starters. A second member completes the same day — still no second card.
- **Definition of done:** The first member to finish a day's reading posts passage-grounded, highlight-aware starters to the thread as "Round," exactly once per circle per plan day.
...
- **UI:** Match proposal as a dedicated screen: the proposed circle as an elevated card (plan badge, member avatar chips) with the Gloo explanation styled as a highlighted quote; primary "Join this circle" and secondary "Browse circles instead" actions; the AgentThinking component while matching runs, with matching-specific rotating lines ("Reading your goals…", "Looking across open circles…"). The founding-member path gets a warm, celebratory treatment ("You're starting something new") rather than an empty screen.
...
- **What gets built:** When a circle reaches minimum size (2) and flips to `active`, Gloo generates a personalised opening message referencing something specific from **two members' real onboarding answers**, posted to the thread as a system message attributed to "Round." Fires exactly once per circle (idempotency key on circle). There is no blank "say hi" state — the icebreaker is the first thread content in every new circle.
- **Language (single now, multilingual at Step 28):** the icebreaker is stored as one system message in a single language — the founder's (earliest member's) preferred language, falling back to the joiner's, then English — with `sourceLanguage` recorded on the row. It is an AI-generated system post (`authorId` null), so per brief §5.12 it is **never translated after the fact**: the Translation Agent (Step 27) must skip it, and **Step 28** ("AI messages generated per-language") upgrades this single-language base into one native generation per distinct member language. Nothing here is stranded — Step 28 already lists icebreaker generation in its Touches.
...
- **How to test it:** Create a fresh circle with member A (goal: "understand the Psalms in hard times"), then join B (goal: "build a morning routine"). On B's join, an icebreaker from "Round" appears that identifiably references A's and B's actual answers. Remove and re-add a member in dev: no second icebreaker.
- **Definition of done:** Every circle reaching two members receives exactly one personalised icebreaker referencing both members' real onboarding answers.
...
- **What the digest is not:** it looks **backward at what members said** — it cannot fire in a circle where nobody reflected, because it has nothing to synthesise. Reviving a stalled circle is Step 24A's job (the digest's inverse), and the plain-language summary of the *passage* is Step 25's. The three are distinct: 24 synthesises people, 24A restarts a conversation, 25 explains the text.
...
### Step 24A — Companion Agent: Round takes a turn in a stalled circle
- **What gets built:** A new **Companion** agent (added to the Step 5 registry and the `AgentName` union) that keeps a conversation alive when it stalls. On a **12-hourly sweep** (the Step 23 rail), for each active circle: resolve the current plan day, then take a turn only if **fewer than half the members have reflected for that day** *and* **no digest has been posted for that day** *and* the thread has had **no member message in the last 12 hours** (an actively chatting circle is left alone). Round then reads the day's thread — the Step 20 starters card plus whatever members did say — and posts one short turn that picks up something specific a member said and opens it to the others; with nothing said at all, it re-opens the starters instead. Posted as a system message attributed to "Round" (`kind = "companion"`, `author_id` null), using the system-message identity defined in Step 20. Idempotent per circle per sweep via `agent_runs` with a `period_key` of the form `2026-07-21-am` / `-pm`, so Round may take at most one turn per sweep (up to twice a day) and a re-run of a sweep spends no Gloo call.

  **Escalation first, as everywhere else.** Ordinary thread messages do not pass the Escalation gate (Step 19 gates reflections only), so every member message the Companion is about to read is screened first via the Step 18 agent. Any flag and Round stays **silent** for that circle this sweep — a conversational nudge must never land on top of a disclosure of crisis — and only the reference-only audit row is written. Because the sweep runs with no user request in flight, the flagged author's support card cannot be shown here: **Step 29 must deliver it through the notification surface**, and this is the gap that step closes.

  **Two hard prompt constraints.** Round never names or implies who has not spoken (the no-comparison constraint, brief §8.1 — "we haven't heard from you" is prohibited; "there's more here worth hearing" is fine), and Round never counsels or advises (brief §6 — pastoral weight belongs to the Escalation support card, not to a follow-up). Warm and short: a couple of sentences and one open question.
- **UI:** No new components — the turn renders through the Step 20 system-message frame (Round mark, "Round" attribution, tinted full-width surface), the same identity the icebreaker and digest use.
- **Why this step comes here:** It is the digest's inverse and shares its rail, so it is placed alongside the 24/25 pair rather than renumbering later steps. Needs the cron rail and `agent_runs` (23), the thread and its system-message rendering (17/20), the Escalation agent (18), and reflections (19) to count participation.
- **Touches:** Companion agent (new module + registry), Gloo Completions V2, Escalation agent, cron route + `vercel.json`, thread UI (one new message kind), database (`agent_runs`, `messages`).
- **How to test it:** In a circle of 4, post one member message for the current plan day and submit no reflections. Trigger the Companion sweep from the Agent Console: a Round card appears picking up that member's actual words and inviting the others in, naming nobody as absent. Trigger the same sweep again: "already ran", nothing written, no new `agent_logs` row. Have 3 of the 4 submit reflections, advance to the next sweep period and trigger again: Round stays silent (the digest owns that day instead). Post a message with the Step 18 crisis test phrasing and trigger: Round posts nothing for that circle, and `escalation_audit` gains a reference-only row with no message content anywhere. Call the cron route without the bearer secret: 401.
- **Definition of done:** A stalled circle receives at most one short, passage-and-conversation-grounded turn from Round per sweep, never when the circle is already talking, never when a digest covers that day, and never in a thread carrying an unaddressed crisis signal.

### Step 26 — Prayer Agent: personal prayer on its own tab
- **What gets built:** A dedicated **`/prayer` tab** (a 5th nav item — one entry in the `app-shell.tsx` `tabs` array), deliberately separate from the circle thread so a prayer never reads as shared. Two generation modes, both private and first-person **singular** ("I pray…"):
  - **Prayer for the day (default):** one tap. Grounded in the user's OWN private material — onboarding answers, their recent unflagged reflections, imported/in-app highlights, and today's reading — gathered by `src/lib/prayer-context.ts`. No circle discussion is used.
  - **Custom prayer (on request):** the user types an intent ("for a good interview tomorrow"); the prayer is led by that request and lightly coloured by the same private context. The free-text prompt is user-authored AI input and a crisis surface, so it passes the **Escalation gate first** (same gate as reflections); a flagged prompt shows the private support card and no prayer is generated.

  The prayer is **streamed** via Completions V2 `stream` (SSE) — `chatCompletionStream()`, a new extension of `src/lib/gloo.ts` that writes the `agent_logs` row once on stream completion; existing call sites are untouched. Shown privately with copy/save. Saved prayers live **on the Prayer tab** — Path A in the `saved_prayers` table, Path B in sessionStorage for the browser session (no database row). Attribution: "Round — a prayer for today" / "Round — a prayer at your request".

  **Optional share to circle:** the user may share a prayer. Before it posts, the agent **recasts** it into an intercessory form that NAMES the author — "I pray for a good interview" → "I pray that {name} will have a good interview" — via `/api/prayer/recast`. The user sees the recast preview and confirms ("Your circle will see this"); on confirm the recast text passes the Escalation gate and posts to the active circle as a distinct **member-authored `shared_prayer`** message card. Nothing is ever auto-shared. Because the card is member-authored, Step 27's Translation Agent covers it for free once it ships.
- **UI:** The Prayer tab has a "Pray for today" hero card, a custom-request card (textarea + "Write this prayer"), and a saved-prayers list. Generation opens a focused bottom sheet: AgentThinking covers the brief pre-stream wait, then the prayer **streams in word by word** (a real thinking effect); copy/save enable only once the stream completes, with toast feedback (toast added to the library). Share shows a recast preview + confirm inline in the sheet. On stream interruption, fall back to a single non-streamed retry behind AgentThinking. In the circle thread the shared prayer renders as a soft-accent praying-hands card ("Shared by {name} · composed with Round").
- **Why this step comes here:** On-demand, no cron dependency. Grounding is the user's own private data (7/10/14/19) + today's reading (11), not circle content, so it needs no digest.
- **Touches:** Prayer agent (daily/custom/recast), Gloo Completions V2 streaming, nav tab, `/prayer` page + screen + sheet, `/api/prayer` (list/save), `/api/prayer/generate` (SSE), `/api/prayer/recast`, `/api/prayer/share`, Escalation reuse, thread UI (`shared_prayer` kind), database (`saved_prayers`).
- **How to test it:** Open the Prayer tab: tap "Pray for today" → a first-person prayer streams in, reflecting your own material + today's reading, in your language; copy and save work; it appears in the saved list. Type "for a good interview tomorrow" → a prayer centred on the request streams in. Type crisis-phrased text → the support card shows, no prayer. Tap Share → the recast preview names you ("I pray that {name}…"); confirm → it posts to your circle as a distinct prayer card; a second member sees it in the thread and nothing is posted when you don't share. Anonymous: both modes work and save for the session; Share is absent.
- **Definition of done:** Any user can generate, copy, and save a private personal prayer (daily or from a request), with the free-text prompt Escalation-gated; and may optionally share a prayer that Round recasts to intercede for them by name, posted only on explicit confirm and never automatically.

### Step 27 — Translation Agent: multilingual thread
- **What gets built:** On every new user-generated message or reflection post: source language detection via a **two-tier cascade** — first a cheap English regex check (case-insensitive match on common English markers such as `\bI was\b`, `\bI am\b`, `\bI have\b`, `\bthe\b`, `\band\b`, `\byou\b`; enough hits ⇒ treat the source as English and **skip the Gloo call entirely**); only if the regex is inconclusive does it fall back to one Gloo detection call, and only if *that* fails to the author-profile-language default (per the Decisions section). For each *distinct* preferred language among circle members that differs from the source, one Gloo translation call with the faith-context prompt; results written to `message_translations` (additive — originals untouched). **System/AI-authored posts (`authorId` null — the Step 20 starters, the Step 22 icebreaker, and the Step 24/24A/25 digest/companion/summary) are explicitly excluded from this agent** (brief §5.12: AI content is never translated after the fact); their per-language delivery is Step 28's job, so this agent's query filters to member-authored rows only. Step 26's shared-prayer card is member-authored (`kind = "shared_prayer"`), so it is covered here automatically with no extra work. Cache rule: a message is never re-translated for the same target language. Thread rendering: each reader sees messages in their own language by default with a "Translated by Round" label and a "Show original" toggle; pending translations show the original with a "Translating…" badge until the next poll swaps them. **Scripture references inside messages pass through as-is; Bible text is never an input to this agent.** Runs post-persist via `waitUntil` so posting is never blocked.
...
### Step 29 — Reminder Agent and in-app notifications
- **What gets built:** An in-app notification model (bell + list, fed by polling). The daily Reminder sweep (on the Step 23 rail) generates via Gloo: a **reading reminder** for any user 2+ days behind their plan (referencing the missed passage and what their circle discussed) and a **message notification** for unread circle messages older than 24 hours (short summary of what was missed). Delivery is **in-app only — no email at all**; all notifications land on the bell + list surface. Idempotent per user+type+day. Instant Access users receive no persisted notifications (no persistence), per the brief.
- **UI:** Notification bell in the top header bar with an unread-count badge; the notification list as card rows with a type icon, short text, relative time, and unread state, plus an empty state ("You're all caught up").
- **Why this step comes here:** Needs plan progress (14), thread activity (17+), digests for context (24), and the cron rail (23).
- **Touches:** Reminder agent, Gloo Completions V2, cron route, database (`notifications`), notification UI.
- **How to test it:** Locally: set a test user's progress 3 days behind, trigger the Reminder sweep from the Agent Console — an in-app notification appears naming the missed passage. Trigger the sweep again: no duplicate in-app notification. Confirm no outbound email is attempted anywhere in the sweep.
- **Definition of done:** Behind-schedule users get personalised reminders and inactive readers get message summaries, delivered in-app only, exactly once per day.

## Phase 7 — Instant Access End-to-End

### Step 30 — Public demo circle with a live AI member, and full anonymous participation
- **What gets built:** A system-owned, always-open **public circle** named "Public Round Circle — Psalms" (the `[<Scripture>]` naming leaves room for more per-scripture public circles later; Psalms ships first because it is the RAG-grounded corpus of Steps 32–33, so its digests showcase best). Two additive schema flags carry it: `circles.kind` (`public` — distinguishing it from matched member circles) and `users.is_bot`. A seed script provisions the circle and its founding members: **(1) an AI bot user** (`is_bot = true`, a warm named persona with its own profile language) added as a real member with a real membership row — not a fake seed row; **(2) Radoslaw**, a real Path A account, seeded as a persistent human member (or he joins manually) so the circle always keeps a named human presence that survives daily pruning. At seed time, via **real Gloo calls**, the bot posts a hello/welcome message, posts one reflection, and completes **at most one reading day** of the plan (so the circle reads as an active, in-progress group, never a finished one); the Facilitator/Summary sweep then produces the first digest, lesson summary, and starters from that content.

  **The bot participates like a normal member.** A new **Circle Bot agent** (registered into the Step 5 framework, precedent: Step 35B's answered-detection agent) drives it: when a member posts a reflection or thread message in the public circle, the bot generates a **member-authored** reply via Gloo (authorId = the bot user, so it renders as an ordinary member post and the Step 27 Translation Agent covers it for free — it is persona content, not Round's `authorId`-null facilitation output, so §5.12's "AI content is never translated after the fact" does not apply). The bot reply runs **post-persist via `waitUntil`** so posting is never blocked, is **rate-limited** (at most one bot reply per short window per circle; the bot never replies to its own or another bot's posts, so it cannot loop), and — like every user-facing turn — screens the triggering content through the **Escalation Agent first**: any flag and the bot stays silent that cycle (a conversational reply must never land on a crisis disclosure). The bot is exempt from every user-targeting sweep (Reminder, Health, answered-prayer detection) — it never receives reminders or nudges.

  **Anonymous users join and take part in the full reading-and-discussion loop.** An Instant Access visitor lands with the public circle joinable in one tap (the only circle an anon session can join — no matching pool, per Path B). Once a member they read the digest, lesson summary, starters, and the live thread; they **submit reflections** (through the Escalation Agent exactly as normal, appearing under their "Reader #n" name) and **post and reply to thread messages** (conversing with the bot, Radoslaw, and any other live members). The bot may reply to them within a poll cycle, so the circle feels alive. Anonymous reflections and messages are stored tagged with the anonymous session ID (pruned in Step 31). **Judges** tap in the same way and become live members of the same shared circle, seeing one another's and the bot's activity in real time.

  **Circle-kind exclusions (so the public circle behaves):** `kind = public` exempts the circle from the circle-matching pool, from the size < 5 cap (it is unbounded), and — critically — from the **Health Agent** sweep (Step 34), which would otherwise flag its many departed anonymous "silent members" or try to merge/archive the showcase. The daily Facilitator/Summary/Companion (and Context, on the Psalms plan) sweeps **do** run on it for real, so its AI thread content is genuinely regenerated, not frozen seed data.

  The upgrade prompt ("Save your progress — sign in with YouVersion") stays reachable from every screen and completes into a real Path A account. **Deferred to Step 30A** — the additional platform features for anonymous members (personal prayer, saved prayers, in-passage highlights, and the notification bell): this step delivers join + read + reflect + converse only. **Confirmed absent for anonymous users:** circle matching, Echoes, and persisted reminders.
- **UI:** This is the judge-facing showcase — a full visual dress rehearsal of the Step 17–25 thread. Seeded content must read naturally (a believable bot persona name and voice, varied message lengths, a believable digest); the bot's live replies must be **indistinguishable in the thread from any member post** (member card, avatar, "Translated by Round" affordances when applicable) — no "I am a bot" chrome. Each screen offers one obvious next action so a first-time visitor never stalls, and the upgrade banner stays persistent but unobtrusive with the OAuth handoff styled as a proper flow, not a bare redirect. Fix any visual seams found during the rehearsal as part of this step.
- **Why this step comes here:** Every capability it stitches together (passage, prompts, reflections, escalation, thread messaging, translation, digest, summary) now exists; the Step 8 session abstraction means anonymous membership is the same code path as authenticated membership, so the only genuinely new code is the two schema flags, the Circle Bot agent, and the circle-kind exclusions. It precedes Step 31, which turns the seed into a daily-refreshed, self-pruning guarantee.
- **Touches:** Seed script, `circles.kind` + `users.is_bot` migration, Circle Bot agent (new, in the Step 5 framework), anonymous membership/join flow, Escalation pipeline, Translation Agent, circle-matching + Health + Reminder exclusions, upgrade prompt UI, OAuth handoff.
- **How to test it:** Full dress rehearsal in an incognito window on **production**: one tap in → optional language pick → pre-reading prompts → live passage → finish reading → starters → open the public circle → read the digest, the bot's hello and reflection, and prior activity → submit a reflection (appears as "Reader #n") → post a thread message → within a poll cycle the **bot replies** as an ordinary member (verify the reply is a fresh live Gloo call in `agent_logs`). Open a second incognito window as another "Reader", post a message, and confirm the first window sees it on its next poll — two live anonymous members share the one circle. Set the browser language to Spanish: the bot's messages render translated with "Show original" (member-authored), while the digest renders natively per Step 28. Submit a crisis-phrased reflection anonymously: the support card appears, nothing surfaces in the thread, and the bot stays silent. Confirm the public circle never appears in Health sweep output and the bot never receives a reminder. Every AI response along the way is live (verify `agent_logs` timestamps).
- **Definition of done:** A brand-new visitor joins the shared public circle in one tap with zero sign-up and completes the full read-reflect-**converse** loop with a live AI member that replies like a normal user; the bot and human seed members are real member rows; all AI calls are live; the circle is excluded from matching, size limits, and Health; and any visitor can upgrade to a full account at any point.

### Step 30A — Full platform features for anonymous circle members (session-scoped)
- **What gets built:** Every remaining member capability, made available to an anonymous session inside the public circle, **session-scoped** (each row tagged with the anonymous session ID) and therefore ephemeral — honouring the brief's "no data persisted between sessions" while making the current session fully functional. Concretely, the anonymous paths of already-shipped features are enabled and verified: **personal prayer** (the Step 26 Prayer tab — generate and **save** prayers for the session), **in-passage highlights** (the Step 14 create/save-highlight path, session-scoped — distinct from the Step 7 YouVersion OAuth *import*, which is inherently unavailable to an anon session with no YouVersion account), and the **notification bell** (Step 29, **relaxing that step's anonymous exclusion to session-scoped only** — session-scoped in-app notifications so the bell surface works in the demo, e.g. a bot/thread-reply notification; still no *persisted* notifications, consistent with the brief). Each feature already carries an authenticated path; this step routes the anonymous session through the same code via the Step 8 resolver, adding only the session-ID tagging and session-scoped storage where a feature previously assumed a persistent user row. Nothing here creates a `users` row and nothing outlives the session; all of it is pruned by the Step 31 job on the same rail as anonymous reflections and messages. (Prayer *requests* and prayer *acts* carry their own anonymous paths in Step 35A and are not duplicated here.)
- **UI:** No new surfaces — the Prayer tab, in-passage highlight affordance, and notification bell already exist from Steps 26/14/29; this step makes them render and function for an anonymous session exactly as they do for an authenticated one, with no "sign in to use this" gate on the deferred features.
- **Why this step comes here:** It builds directly on Step 30's anonymous membership and keeps that step focused on the core join-read-reflect-converse loop. Splitting the "everything else works too" surface into its own step keeps each independently verifiable and isolates the session-scoping work (which touches several earlier features) from the circle/bot work. It precedes Step 31 so the pruning job covers all anon-generated data — prayers, highlights, and notifications included — from the moment those anon paths exist.
- **Touches:** Step 26 Prayer (anon session path), Step 14 highlights (session-scoped create), Step 29 notifications (session-scoped), the Step 8 session resolver, session-ID tagging on those tables.
- **How to test it:** As an anonymous visitor in the public circle: open the Prayer tab, generate and save a personal prayer — it persists for the session and is gone after the session ends; create an in-passage highlight on the reading screen — it appears in the session and is imported to no account; trigger an event that produces a notification (e.g. the bot replies to your message) and confirm the bell shows it. End the session and reopen: none of it survives, and a fresh entry works again. Trigger the Step 31 prune and confirm anon prayers, highlights, and notifications are removed alongside anon reflections, while the seeded bot/Radoslaw content remains.
- **Definition of done:** An anonymous circle member can use personal prayer, saved prayers, in-passage highlights, and in-app notifications fully within the session; every such artifact is session-scoped, creates no user row, and is pruned by Step 31.
...
### Step 32 — RAG corpus: Psalms commentary ingestion via Data Engine
- **What gets built:** A processing script that extracts the provided Psalms commentary PDF to text and splits it into **one Markdown file per Psalm** — Markdown rather than PDF slices because Gloo's splitter cuts on headings, paragraphs and section breaks, and "well-structured text produces better chunks than flat text". Each file opens with the canonical reference as a heading (`# Psalms Commentary — Psalm 23`, plus verse range) so the reference lives in the retrievable chunk text itself, not only in metadata. A companion upload script ingests the files under a single publisher (the `rag_publisher` value) via `POST /ingestion/v2/files` (multipart), setting `item_title` to the naming convention and carrying source and licence in `item_summary` / `item_tags`, with a stable per-Psalm `producer_id` (e.g. `psalms-commentary-psalm-23`) so re-runs deduplicate instead of creating copies. Ingestion is asynchronous — "the upload response means your file is queued, not searchable" — so the script polls each item through `QUEUED` → `CHUNKING` → `COMPLETED` and only reports success when every item is COMPLETED. The Step 4 `groundedCompletion()` placeholder in `src/lib/gloo.ts` is completed against the grounded endpoint (`/ai/v2/chat/completions/grounded`, plain `fetch` — the OpenAI SDK cannot reach this path): standard completion params plus `rag_publisher`, `sources_limit` (1–10, default 3), `include_citations: true` (the API defaults it to `false`), `tradition`, one routing choice, and typed response fields `citations[]` (`item_title`, `item_url`, `author`, `publisher`, `publication_date`, `snippets`) and `sources_returned`. A dev-only route issues a grounded completion for a given passage reference and returns the completion plus its citations.
- **Why this step comes here:** The Context Agent (33) is only as good as its corpus; grounded retrieval must be proven independently before it feeds the Facilitator. Depends on Step 4.
- **Why per-Psalm and not one upload:** chunking cannot be disabled or tuned and overlapping paragraph-sized chunks would bleed across Psalm boundaries; more decisively, attribution is item-level, so a single item makes every citation read "Psalms Commentary" and leaves the Step 33 scope guard and the digest attribution line with nothing to check. Full reasoning in the RAG architecture decision above.
- **Corpus growth (optional, later, no code):** because retrieval is publisher-scoped, adding public-domain commentary for other books (e.g. Matthew Henry, pre-split per book+chapter through the same two scripts and title convention) expands Context Agent coverage from Psalms to the whole Bible with zero code change. Scheduled only if time remains after Step 33; the Psalms corpus alone is the committed scope.
- **Amended after shipping — the Search path was added alongside the grounded one.** Once the 149-item corpus was live it became testable which retrieval mechanism actually scopes to a chapter, and Grounded Completions lost: see the retrieval architecture decision above for the measurements. Step 32 therefore also delivers `searchCorpus()` in the Gloo client (`POST /ai/v1/data/search`, logged to `agent_logs` with a null model since a search has no model or token usage), `src/lib/corpus.ts` holding the passage→filename mapping and the chunk filter, `GLOO_SEARCH_TENANT` (the publisher name normalised with underscores — the hyphen form 403s), and a second dev route `/api/dev/search` that reports retrieved-vs-kept counts so the filter's effect is visible. `groundedCompletion()` and `/api/dev/grounded` stay exactly as built: they are the brief's named RAG capability and remain working and demonstrable. One implementation note worth keeping: the chunk budget selects by **certainty and then restores `part` order** — an earlier part-ordered slice kept a long commentary's opening and dropped the verse the question was about, and the model correctly answered "the commentary does not address this".
- **Touches:** Split script, ingestion/upload script (Data Engine `POST /ingestion/v2/files` + status polling), Gloo client (`groundedCompletion`, `searchCorpus`), corpus scoping helpers, two dev routes.
- **How to test it:** Run the split script against the PDF and eyeball a few files — headings present, no Psalm's commentary trailing into the next file. Run the upload script: it exits only once every item reports COMPLETED, and the per-Psalm items appear in the Content Library in Gloo AI Studio with correct titles. Re-run it: `producer_id` dedupe means the item count does not double. `curl` the dev route for "Psalm 23": the response has `sources_returned: true` and citations whose `item_title` values name Psalm 23 — content demonstrably about Psalm 23 (shepherd imagery, historical notes), not adjacent Psalms. Query a passage with no corpus coverage (e.g. Mark 4): `sources_returned: false`.
- **Definition of done:** The per-Psalm corpus is live and COMPLETED in the Content Library with idempotent re-upload; `groundedCompletion()` round-trips with typed citations; and `searchCorpus()` plus the filename filter return commentary provably scoped to the queried passage — a covered Psalm keeps only its own chunks, a partial-chapter reference maps to its chapter, an uncovered chapter keeps zero, and a non-Psalms passage is answered without a retrieval call at all.

### Step 33 — Context Agent (implementing the Step 5 stub)
- **What gets built:** The Context stub becomes real, on the **Search + Completions V2** path from the Decisions section rather than a one-call grounded completion. When the Facilitator sweep processes a circle, the Context Agent (1) resolves the day's passage to exactly one corpus filename, (2) issues one Search call, (3) keeps only the chunks whose `filename` matches, ordered by `part`, and (4) hands them to one Completions V2 call that **restates the commentary in plain modern language** in the circle's language. If no chunk survives the filename filter the agent returns its existing no-op result and the Facilitator runs ungrounded — exactly the stub's path, and now a deterministic decision rather than an inference from `sources_returned`. When commentary does survive, the modernised text is handed to the Facilitator as additional grounding and the digest card gains a source attribution line built from the chunks' `item_title` (e.g. "Historical context from Treasury of David, public domain"). With the Step 32 corpus this means Psalms digests are grounded and everything else is not — the gate is corpus coverage, so later corpus growth extends coverage with no code change here. No interface changes to the Facilitator beyond consuming the optional grounding — proving the stub architecture worked. A new `searchCorpus()` method joins `groundedCompletion()` in the Gloo client; both are logged to `agent_logs` like every other Gloo call.
- **Two Gloo calls, not one, and why that is the better trade.** The one-call grounded path stays shipped and demonstrable from Step 32, but Context uses Search first because scoping becomes identity-based instead of similarity-based. Measured on the live corpus: a filename-filtered search recovers **11,636 chars of the Psalm 23 commentary (74% of the item)** against 1,075 at `sources_limit: 3`; uncovered Psalm 119 yields **exactly 0** on-target chunks where `sources_returned` reported `true` with Psalms 106/118/128/149 citations; and `part` restores reading order before prompting. The extra call costs one cheap search plus a larger completion prompt — acceptable at once per circle per day.
- **Passage → filename mapping is the whole correctness surface.** `planDay.label` ("Psalm 23", already loaded by `runCircleDigest`) is parsed to book + chapter and mapped to `psalm-NNN.md`, **ignoring any verse span** so a partial-chapter day like "Psalm 139:1–18" still maps to `psalm-139.md`. The USFM `reference` is used only to fetch the passage and never reaches Gloo: `PSA.23` matches nothing in the corpus, and under the old grounded path its `sources_returned: false` was indistinguishable from a real gap. The search `query` names the passage in human form ("Psalm 23 commentary") and takes `limit: 100`, `certainty: 0` — over-fetch wide, then filter, since server-side filtering does not exist. A weak query can now only cost recall, never correctness.
- **Chunk budget:** the top ~10–15 surviving chunks (~4–5k chars) go into the restatement prompt, ordered by `part` — enough for a 3–4 sentence digest grounding without paying for the whole item. The search query is built from the passage alone: **no reflections and no circle themes**, both because Context's job per brief §5.13 is the passage's historical context and word notes (the Facilitator synthesises what members said) and because a theme-laden query measurably drifts (1 of 7 citations on-chapter, into the lament psalms). A useful property follows: the retrieval is a pure function of `(reference, language)` and identical for every circle on that plan day, so it can be cached per passage and reused rather than repeated per circle.
- **UI:** The source attribution renders as a subtle footer line inside the digest card (small text, muted colour) — present when grounding was used, absent otherwise, never competing with the digest content.
- **Why this step comes here:** Slots between corpus (32) and the long-established Facilitator (24). Matches the brief's "before Facilitator" trigger; "Psalms only" is realised as corpus coverage rather than a hardcoded book check.
- **Touches:** Context agent, Gloo client (`searchCorpus()`), a shared passage→filename mapping helper (reused by 33A), Completions V2 restatement call, Facilitator input, digest UI (attribution line), `GLOO_SEARCH_TENANT` env var.
- **How to test it:** Run the Facilitator sweep for a circle on a Psalms plan day: the digest includes commentary-informed context in modern language and shows the attribution line; `agent_logs` shows the Context search and restatement calls before the Facilitator run. Run it for a circle reading Mark: no chunk matches `mark-NNN.md`, Context returns its no-op, the digest has no attribution line. Run it on a Psalm 119 day — a covered *book* with an uncovered chapter, the case the old `sources_returned` gate got wrong: 0 surviving chunks, no grounding, no attribution. Run a partial-chapter day ("Psalm 139:1–18"): maps to `psalm-139.md` and grounds normally, proving the verse span is ignored. Inspect the chunks the agent kept: every one has `filename === psalm-NNN.md` for the day's Psalm, and none from a neighbouring Psalm.
- **Definition of done:** Passages with corpus coverage produce RAG-grounded digests with source attribution and modernised commentary language; uncovered passages — including uncovered chapters inside a covered book — produce untouched digests via the stub's no-op path; and every chunk that reaches a prompt provably belongs to the day's passage.

### Step 33A — Ask about this passage (grounded Q&A on the reading screen)
- **Scope note:** This step is **beyond the brief** — nothing in the requirements asks for passage Q&A. It exists because Step 33's grounding, though required by §5.13, is nearly invisible in a demo: one muted footer line, once per circle per day, only on covered passages, and only after the reflection threshold is met. This step makes the same corpus user-initiated and immediately visible, and it costs no new infrastructure. It is scheduled **after** 33 so no specified feature is traded for an unspecified one.
- **What gets built:** An "Ask about this passage" affordance on the reading screen, on the same **Search + Completions V2** path as Step 33 and reusing its passage→filename mapping. The user's question goes through the **Escalation Agent first** — same non-negotiable ordering as a reflection, since a free-text box on a Bible passage is a likely place for crisis language — and a flagged question shows the existing support card and issues no retrieval or completion call at all. Otherwise the question itself becomes the search `query` (measured: a verse-level question returns 15 targeted chunks, ~6.8k chars, from the right item), the results are filtered to the passage's one filename, and the survivors are answered over by one Completions V2 call whose system prompt forbids answering outside the retrieved commentary. Zero surviving chunks — an uncovered book, or an uncovered chapter like Psalm 119 — produces a plain "no commentary covers this passage" state rather than a general-knowledge answer, and the deterministic filter means that state is a fact rather than a guess. Answers carry the `item_title`-derived attribution line and are session-scoped for everyone, signed-in and anonymous alike: no new table, no history — the `agent_logs` rows the client writes are the audit trail. A per-session daily cap and a single-in-flight rule bound the cost, since unlike the digest this is a paid round trip per tap.
- **Question-driven retrieval is where the filename filter earns the most.** The digest's fixed query is stable, but a reader's question drags semantic retrieval across the whole Psalter — "why does the psalmist call God a shepherd?" landed 1 of 6 citations on-chapter under the grounded path, drifting into Psalms 80/100/54/50/113 where shepherd language also lives. Filtering on identity removes that failure mode entirely instead of mitigating it. Surviving-chunk counts are logged from day one so `limit` can be tuned against real questions rather than guessed.
- **UI:** Collapsed by default under the passage so Scripture stays the focus of the screen, expanding to a single-line input. `AgentThinking` while the call runs ("Looking through the commentary…"), then an answer card with the source footer. The no-coverage state is a calm one-liner, not an error. Never covers or competes with the passage text, and the cap is communicated as a friendly limit rather than a failure.
- **Why this step comes here:** Depends on Step 32's corpus and on Step 33's `searchCorpus()` method and filename mapping, so it cannot precede either. Placed immediately after 33 because it is the demo-facing payoff of the same RAG work.
- **Touches:** Reading screen UI, a question API route, Escalation Agent (existing gate), Gloo client (`searchCorpus()` + Completions V2), Step 33's shared passage→filename mapping, rate limiting.
- **How to test it:** On a Psalms plan day, ask "why is the shepherd image used here?" — the answer is drawn from the commentary and the footer names "Treasury of David — Psalm 23". Ask something the commentary does not address on the same day: the answer says so instead of inventing. Switch to a Mark 4 day and ask anything: no chunk matches the passage's filename, so the no-coverage line appears and nothing is fabricated. Same on a Psalm 119 day — a covered book with an uncovered chapter. Inspect the chunks used for any answer: all carry the day's `psalm-NNN.md`, never a neighbour's. Submit a question containing crisis language: the support card appears, `agent_logs` shows the Escalation call and **no** search or completion call. Ask past the daily cap: the friendly limit message, no call. Repeat the whole flow as an Instant Access user: fully functional, nothing persisted.
- **Definition of done:** A user can ask a free-text question about the passage they are reading and get an answer grounded in the commentary corpus with source attribution, an honest no-coverage response when the corpus does not cover the passage, every chunk used provably belonging to that passage, Escalation screening ahead of every call, and cost bounded by a per-session cap — with anonymous parity and no persistence.
...
- **UI:** A polished card-flip deck: full-width flip card with front/back faces styled from the design tokens, tap to flip, "card x of y" progress indicator, paired "Know it" / "Review again" actions with a clear pressed state, an end-of-deck completion screen, and the AgentThinking component while the deck generates ("Drawing out key verses…") — this is a showcase interaction, not a plain list.
...
## Phase 8A — Prayer Requests

### Step 35A — Prayer requests: create, share, pray, answer
- **What gets built:** The prayer-request core per the Decisions entry: `prayer_requests` and `prayer_acts` tables (additive migration), a "Prayer requests" section where a user writes a request (private by default), a per-request **"Share with circle"** action, and the shared-request card in the circle thread. Every submitted request text runs through the **Escalation Agent first** — same pipeline position as reflections; a flagged request is saved private to its author, is never shareable, and only its ID is audit-logged. Shared cards go through the **Translation Agent** exactly like member messages (async, cached, "Show original" toggle). Members tap **"I prayed"** once per request; the card shows the author an anonymous count only ("3 people prayed for this") — no names anywhere in UI or API responses. The author (only) can mark a request **answered**; the card updates and the request moves to the answered timeline (Step 35B). Instant Access: anonymous members can tap "I prayed" on others' requests (acts keyed to the anonymous session ID and pruned by the Step 31 job); their own requests are session-scoped like their other data.
- **UI:** Request composer with a clear private/shared state and an explicit share confirmation ("Your circle will see this request"); the shared card visually distinct from member messages (soft accent, praying-hands motif from the icon set) with the count line and, for the author, a quiet "Mark as answered" action; answered cards get a subtle "Answered" badge with the date.
- **Why this step comes here:** Composes existing rails — thread (17), Escalation (18/19), Translation (27), anonymous sessions (8) — without new infrastructure; placed with the other post-core feature work so the daily loop ships first.
- **Touches:** Database (`prayer_requests`, `prayer_acts`), request UI, thread card, Escalation pipeline reuse, Translation pipeline reuse, session resolver (Path B rules).
- **How to test it:** Create a request: it is visible only to you. Share it: it appears in the thread as a request card; a Spanish-preference member sees it translated with "Show original." Tap "I prayed" from two other accounts: the author sees "2 people prayed for this," no names; tapping again from the same account does not increment. Author marks it answered: badge appears; other members cannot mark it. Submit a request with crisis language: the support card shows privately, the request cannot be shared, and only its ID is in the audit log. As an anonymous user: pray on an existing request (count increments), create a request, end the session — the request is gone after the Step 31 prune.
- **Definition of done:** Members can create private requests, share them per-item to the circle, pray anonymously (count only, never names), and authors alone mark requests answered — with Escalation gating every request and translations applied to shared cards.

### Step 35B — Answered-prayer detection and the testimony layer
- **What gets built:** Three pieces on top of 35A. **(1) Answered-detection sweep:** a new detection agent (registered in the Step 5 framework) runs on the daily cron: for each user with active requests older than ~7 days, Gloo compares the request text against the user's recent reflections and newer requests; on a likely match it writes a **private suggestion notification** ("Three weeks ago you asked prayer for your job interview — today's reflection mentions starting work. Mark as answered?"). It never auto-marks; the author confirms or dismisses, and a dismissed suggestion never re-fires for the same request+evidence pair. Idempotent via `agent_runs` on `(answered_detection, user_id, period_key)`. **(2) Gratitude moment:** on marking answered (manually or via suggestion), Gloo offers a 1–2 sentence thanksgiving line in the author's language, postable to the circle or discarded — optional, never automatic. **(3) Answered timeline:** a profile section listing answered requests chronologically (original text, date asked, date answered, prayed-count), private to the author. Also in this step, the **Facilitator prompt gains a celebration input** — when a shared request gathered prayer acts that day, the digest may note it ("the circle prayed together for Maria's request") as celebration, never a tally. (The Step 26 Prayer Agent is a private, personal prayer grounded in the user's own material, not the circle's shared requests, so it is no longer extended here — prayer *requests* (35A) are a separate, shared object.) Path B: no detection, no timeline (no persistence).
- **UI:** The suggestion arrives through Step 29's notification surface as a gentle card with "Mark as answered" / "Not yet" actions; the gratitude line appears in a small sheet with post/discard; the profile timeline renders answered requests as quiet testimony cards.
- **Why this step comes here:** Needs 35A's tables and statuses, reflection history (19), the cron rail (23), and notifications (29). This is the feature's AI substance: Gloo connecting a weeks-old request to today's reflection — memory over time.
- **Touches:** New detection agent module, cron route, notifications, Facilitator prompt (celebration input), profile UI, `agent_logs`.
- **How to test it:** Seed an active request "pray for my exam" back-dated two weeks; submit a reflection "passed my exam, so relieved"; trigger the detection sweep from the Agent Console: a private suggestion notification appears and `agent_logs` records the call. Accept it: status flips to answered, the gratitude sheet offers a line, and the request shows on the profile timeline with its prayed-count. Dismiss instead: the next sweep produces no repeat suggestion. Re-trigger the sweep twice: no duplicates. Generate a prayer while a shared request is active: the prayer references the request's burden. Run the Facilitator after members prayed on a shared request: the digest includes a celebration mention, no counts-per-member anywhere.
- **Definition of done:** Authors receive private, evidence-based "mark as answered?" suggestions that never auto-mark, can post an optional gratitude line, see their answered timeline on profile, and circle prayers and digests are grounded in the circle's real shared requests.

## Phase 9 — Tier 3 Scoping and Final Audit

### Step 36 — Tier 3 future-directions writeup and constraint audit
- **What gets built:** Documentation, not features. The technical writeup names and scopes the Tier 3 directions: **Echoes** (Memory Agent scoring old highlights against current circle activity to resurface a relevant past highlight — the Memory stub from Step 5 remains its landing point), **Bridge Agent** (extended catch-up experiences beyond the Health Agent's bridge summary), **Retrospective** (end-of-plan circle journey recap), and **Verse of the Day** (home-screen touchpoint). Alongside it, a final constraint audit of the codebase: grep-verified absence of any direct OpenAI/Anthropic/other-provider calls (judges will check); no hardcoded Bible text anywhere; every surface rendering Bible text displays the version's copyright attribution; no code path feeding Bible text into the Translation agent; Escalation ordering intact in the reflection pipeline; Tier 2/3 stubs and implementations consistent with the Step 5 interfaces; no UI surface exposing inter-member pace or streaks; and every corpus document in the Content Library carries its source and licence in metadata (public-domain verification for all ingested commentary). The audit also includes a UI consistency sweep: every screen composes from the Step 8A library and tokens (no one-off colours/type/spacing introduced by later steps), and every screen has designed loading, empty, and error states per the post-8A UI standard.
...
1. **Path A:** YouVersion OAuth sign-in with highlight consent → onboarding (Spanish/NVI, goals) → AI-generated plan (validated refs) → AI match into a circle → pre-reading prompts → live NVI passage + highlight → finish → starters in thread → reflection (Escalation-gated) → next-day digest + summary via cron → prayer → share a prayer request; second account taps "I prayed" (author sees count only, no names) → mark it answered; gratitude line offered → cross-language thread with a second English-preference account (translations cached, originals toggleable) → fall 2 days behind → in-app reminder notification arrives on the bell.
2. **Path B:** Incognito, one tap → defaults → full read-reflect loop in the demo circle as "Reader #n" → tap "I prayed" on a shared demo request (count increments, no name recorded) → flashcards → upgrade to Path A mid-session.

Plus the idempotency drill: trigger every cron route twice back-to-back from the Agent Console and confirm zero duplicate digests, summaries, reminders, icebreakers, Health actions, or answered-prayer suggestions.
