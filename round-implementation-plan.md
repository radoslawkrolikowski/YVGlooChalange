...
- **How to test it:** Full dress rehearsal in an incognito window on **production**: one tap in → optional language pick → pre-reading prompts → live passage → finish reading → starters → open demo circle → read digest and reflections → submit a reflection (appears as "Reader #n") → open the Prayer tab and pray for today → tap upgrade and complete YouVersion sign-in. Separately, submit a crisis-phrased reflection anonymously: the support card appears, nothing surfaces in the thread. Every AI response along the way is live (verify `agent_logs` timestamps).
