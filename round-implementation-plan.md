...
**RAG architecture: one-call Grounded Completions against a publisher-scoped corpus; whole-Bible growth is data ops, not code.** Grounded Completions is its own endpoint — `POST https://platform.ai.gloo.com/ai/v2/chat/completions/grounded`, not reachable through the OpenAI SDK (our client already uses plain `fetch`, so this costs nothing) — taking the standard Completions params plus `rag_publisher` (names our corpus, case-sensitive), `sources_limit` (1–10, default 3), `include_citations` (**defaults to `false`**, so it must be passed explicitly), `tradition` (`evangelical` / `catholic` / `mainline` / `not_faith_specific`), and exactly one routing choice (`auto_routing` / `model_family` / `model`). Retrieval and generation happen in a single call; the response returns a `citations` array (`item_title`, `item_url`, `author`, `publisher`, `publication_date`, `snippets`) plus a `sources_returned` boolean. In the streaming variant citations arrive as the first SSE chunk — not needed by any step here, which uses the non-streamed call.

**The corpus is pre-split per Psalm because Gloo's chunking is forced and item-level attribution is the only scope signal.** Ingestion always chunks — "there is no setting to disable it, bypass it, or tune its size and overlap" — into roughly paragraph-sized chunks with deliberate overlap between consecutive chunks. The chunk is the retrieval unit but the **parent item** is the attribution unit, so a single monolithic commentary upload would return every citation titled "Psalms Commentary" regardless of which Psalm the chunk came from, blinding both the Context Agent's scope guard and the digest's attribution line, and its overlapping chunks would straddle Psalm boundaries. Gloo's own guidance is the remedy: "if your content must not be merged with a neighbor… upload each unit as its own item." Hence one item per book+chapter (per Psalm for the Psalms corpus), titled by a strict convention ("<Source> — <Book> <Chapter>", e.g. "Psalms Commentary — Psalm 23") **and carrying that same reference as a heading in the item's own text** — retrieval matches chunk text, and whether `item_title` metadata feeds the embedding is unverified, so scoping never rests on metadata alone. There is no metadata filtering and no relevance scores, so the Context Agent verifies retrieved citations' `item_title` against the current passage before use — a client-side guard against cross-book contamination, sharpened by the per-Psalm split from book-level to exact-chapter matching. Splitting makes mis-scoped retrieval *detectable*, not impossible; the guard is what makes it safe — verified live, the query "Psalm 23" returns a Psalm 93 citation alongside the right one, so callers filter the whole citation list rather than trusting the first entry. For the same reason every grounded prompt names the passage in **human form** ("Psalm 23"), never USFM: retrieval is semantic over chunk text, `PSA.23` matches nothing, and its `sources_returned: false` is indistinguishable from a genuine corpus gap. The `sources_returned` flag replaces any hand-built "is this a Psalm" gate: the Context Agent always attempts grounding and simply reports "no grounding" when retrieval finds nothing relevant. Corpus growth beyond the Psalms PDF (public-domain whole-Bible commentaries, pre-split the same way) requires **zero code change** — same publisher, same call — and is scheduled as optional data ops after Step 33 ships, with the Psalms-only corpus always remaining a sufficient fallback.
...
- **Touches:** Split script, ingestion/upload script (Data Engine `POST /ingestion/v2/files` + status polling), Gloo client (`groundedCompletion` completion), dev route.
...
- **Definition of done:** The per-Psalm corpus is live and COMPLETED in the Content Library with idempotent re-upload, `groundedCompletion()` round-trips with typed citations, and a queried Psalm returns correctly-scoped grounded content while uncovered passages report no grounding.

### Step 33 — Context Agent (implementing the Step 5 stub)
- **What gets built:** The Context stub becomes real: when the Facilitator sweep processes a circle, the Context Agent issues **one grounded completion** for the current passage — the prompt asks Gloo to retrieve relevant commentary and **restate it in plain modern language** in the same call (no separate restatement pass). Gating is data-driven, not book-driven: if the response has `sources_returned: false`, or the citations' `item_title` values do not match the current passage's book **and chapter** (the client-side scope guard from the Decisions section — the per-Psalm corpus split makes exact-chapter matching possible, so a Psalm 22 citation is rejected for a Psalm 23 reading, not just a Mark citation), the Context Agent returns its existing no-op result and the Facilitator runs ungrounded — exactly the stub's path. When grounding survives both checks, the modernised commentary is handed to the Facilitator as additional grounding, and the digest card gains a source attribution line built **from the returned citations** (e.g. "Historical context from <item_title/publisher>, public domain"). With the Step 32 corpus this means Psalms digests are grounded and everything else is not — but the gate is corpus coverage, so any later corpus growth (Step 32's optional data ops) extends coverage with no code change here. No interface changes to the Facilitator beyond consuming the optional grounding — proving the stub architecture worked.
- **Reference form is the human label, never USFM — verified.** Retrieval is semantic over chunk text, and USFM strings appear nowhere in the corpus: a grounded query for `PSA.23` returns `sources_returned: false`, exactly as an uncovered book does, while "Psalm 23" retrieves correctly. Since a bad reference form and a genuine corpus gap are indistinguishable in the response, this is a silent-failure trap, so the Context prompt is built from `planDay.label` ("Psalm 23", already loaded by `runCircleDigest`) and the USFM `reference` never enters the prompt. The scope guard parses book and chapter from that label and **ignores any verse span**, so a partial-chapter day like "Psalm 139:1–18" still matches the Psalm 139 item.
- **The guard filters the citation list, not just the first citation.** Verified against the live corpus: the query "Psalm 23" returns Psalm 23 *and* Psalm 93 at `sources_limit: 3` — nearest-neighbour noise with a correct first citation. Checking `citations[0]` would pass while Psalm 93 snippets fed the digest, so every citation is checked and off-chapter ones are dropped; grounding proceeds on what survives, and becomes a no-op only when nothing does.
...
- **Touches:** Context agent, Gloo Grounded Completions (via the completed client method), a shared passage/citation matching helper (reused by 33A), Facilitator input, digest UI (attribution line).
- **How to test it:** Run the Facilitator sweep for a circle on a Psalms plan day: the digest includes commentary-informed context in modern language and shows the attribution line naming the source from the citations; `agent_logs` shows the Context grounded call before the Facilitator run. Run it for a circle reading Mark: `sources_returned` is false (or citations fail the scope guard), Context returns its no-op, and the digest has no attribution line. Force the cross-book case in dev (query a Psalm-adjacent theme from a Mark passage): the citation title guard rejects the chunks and the digest stays ungrounded. Force the wrong-chapter case (a Psalm 23 reading whose citations come back titled Psalm 22): same rejection, proving the guard is chapter-exact and not merely book-level.
- **Definition of done:** Passages with corpus coverage produce RAG-grounded digests with citation-derived source attribution and modernised commentary language; uncovered passages produce untouched digests via the stub's no-op path, with the scope guard verified for both cross-book and wrong-chapter citations.

### Step 33A — Ask about this passage (grounded Q&A on the reading screen)
- **Scope note:** This step is **beyond the brief** — nothing in the requirements asks for passage Q&A. It exists because Step 33's grounding, though required by §5.13, is nearly invisible in a demo: one muted footer line, once per circle per day, only on covered passages, and only after the reflection threshold is met. This step makes the same corpus and the same client method user-initiated and immediately visible, and it costs no new infrastructure. It is scheduled **after** 33 so no specified feature is traded for an unspecified one.
- **What gets built:** An "Ask about this passage" affordance on the reading screen. The user's question goes through the **Escalation Agent first** — same non-negotiable ordering as a reflection, since a free-text box on a Bible passage is a likely place for crisis language — and a flagged question shows the existing support card and issues no grounded call at all. Otherwise one `groundedCompletion()` runs, scoped to the current passage by its human label (per Step 33's verified reference rule), reusing Step 33's shared citation guard: off-chapter citations are dropped, and if nothing survives — or `sources_returned` is false — the UI says plainly that no commentary covers this passage rather than answering from the model's general knowledge, which the system prompt forbids. When grounding survives, the answer renders with a citation-derived attribution line. Answers are session-scoped for everyone, signed-in and anonymous alike: no new table, no history — the `agent_logs` row the client already writes is the audit trail. A per-session daily cap and a single-in-flight rule bound the cost, since unlike the digest this is one paid call per tap.
...
- **Why this step comes here:** Depends on Step 32's corpus and client method and on Step 33's guard helper, so it cannot precede either. Placed immediately after 33 because it is the demo-facing payoff of the same RAG work.
- **Touches:** Reading screen UI, a question API route, Escalation Agent (existing gate), Gloo Grounded Completions, Step 33's shared citation guard, rate limiting.
- **How to test it:** On a Psalms plan day, ask "why is the shepherd image used here?" — the answer is drawn from the commentary and the footer names "Treasury of David — Psalm 23". Ask a question whose answer is not in the corpus on the same day: the answer stays inside the retrieved commentary or reports no coverage; it never invents. Switch to a Mark 4 day and ask anything: the no-coverage line appears and no fabricated commentary is shown. Same for Psalm 119, which the abridgement does not cover — a covered book with an uncovered chapter. Submit a question containing crisis language: the support card appears, `agent_logs` shows the Escalation call and **no** grounded call. Ask past the daily cap: the friendly limit message, no call. Repeat the whole flow as an Instant Access user: fully functional, nothing persisted.
- **Definition of done:** A user can ask a free-text question about the passage they are reading and get an answer grounded in the commentary corpus with source attribution, an honest no-coverage response when the corpus does not cover the passage, Escalation screening ahead of every call, and cost bounded by a per-session cap — with anonymous parity and no persistence.
