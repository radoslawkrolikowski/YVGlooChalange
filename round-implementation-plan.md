...
- **What gets built:** Circle entity with states (forming → active → stalled → archived), size limits (min 3, max 5), and an attached reading plan. Users can create a circle (choosing a plan), browse open circles (name, plan, member count — never member progress), and join one. A circle becomes `active` at 3 members; joining is blocked at 5. Members list shows display names only. **Joining a circle aligns the member's reading with the circle's attached plan:** if the joiner's active plan differs, an explicit prompt ("This circle reads the Gospel of Mark — switch your reading?") makes the circle's plan their active plan and pauses the personal plan (Step 12A — progress preserved, resumable later). Never a silent swap; declining cancels the join. Everything downstream (digests, starters, reminders) assumes circle members share the circle's plan.
