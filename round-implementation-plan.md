...
- **What gets built:** A "Generate a prayer" action in the circle thread. Gloo produces a 5–8 sentence first-person-plural prayer grounded in that day's circle discussion (reflections + digest), attributed "Round — based on today's reading," generated in the requesting user's language — **streamed** via Completions V2's `stream` (SSE) support, added to the shared Gloo client in this step (a small extension of `src/lib/gloo.ts`; no retro change to shipped call sites). Shown privately to the requester with copy and save actions — **not** broadcast to the circle. Saved prayers appear on the user's profile (Path A) or last for the session (Path B). The `agent_logs` row is written once on stream completion, same as non-streamed calls.
- **UI:** The prayer opens in a focused bottom sheet/modal with generous typography and the attribution line styled subtly beneath; the AgentThinking component covers the brief pre-stream wait, then the prayer text **streams in word by word** as Gloo generates it — a real thinking effect fitting the moment, not a simulated one; copy and save give clear success feedback (toast added to the library) and enable only once the stream completes. Saved prayers list on the profile as cards with reference and date. On stream interruption, fall back to a single non-streamed retry behind AgentThinking.
...
- **What gets built:** On every new user-generated message or reflection post: source language detection (one Gloo call, author-profile-language fallback per the Decisions section); for each *distinct* preferred language among circle members that differs from the source, one Gloo translation call with the faith-context prompt; results written to `message_translations` (additive — originals untouched). **System/AI-authored posts (`authorId` null — the Step 20 starters, the Step 22 icebreaker, and the Step 24/24A/25 digest/companion/summary) are explicitly excluded from this agent** (brief §5.12: AI content is never translated after the fact); their per-language delivery is Step 28's job, so this agent's query filters to member-authored rows only. Cache rule: a message is never re-translated for the same target language. Thread rendering: each reader sees messages in their own language by default with a "Translated by Round" label and a "Show original" toggle; pending translations show the original with a "Translating…" badge until the next poll swaps them. **Scripture references inside messages pass through as-is; Bible text is never an input to this agent.** Runs post-persist via `waitUntil` so posting is never blocked.
...
- **What gets built:** Three pieces on top of 35A. **(1) Answered-detection sweep:** a new detection agent (registered in the Step 5 framework) runs on the daily cron: for each user with active requests older than ~7 days, Gloo compares the request text against the user's recent reflections and newer requests; on a likely match it writes a **private suggestion notification** ("Three weeks ago you asked prayer for your job interview — today's reflection mentions starting work. Mark as answered?"). It never auto-marks; the author confirms or dismisses, and a dismissed suggestion never re-fires for the same request+evidence pair. Idempotent via `agent_runs` on `(answered_detection, user_id, period_key)`. **(2) Gratitude moment:** on marking answered (manually or via suggestion), Gloo offers a 1–2 sentence thanksgiving line in the author's language, postable to the circle or discarded — optional, never automatic. **(3) Answered timeline:** a profile section listing answered requests chronologically (original text, date asked, date answered, prayed-count), private to the author. Also in this step, the **Prayer Agent's grounding is extended** (Step 26 call site): the circle's active shared requests join reflections+digest in the prompt, so generated prayers name the circle's real burdens; and the **Facilitator prompt gains a celebration input** — when a shared request gathered prayer acts that day, the digest may note it ("the circle prayed together for Maria's request") as celebration, never a tally. Path B: no detection, no timeline (no persistence).
...
- **Touches:** New detection agent module, cron route, notifications, Prayer Agent prompt (grounding extension), Facilitator prompt (celebration input), profile UI, `agent_logs`.
