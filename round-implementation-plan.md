...
**RAG architecture: one-call Grounded Completions against a publisher-scoped corpus; whole-Bible growth is data ops, not code.** Gloo Grounded Completions is the standard Completions endpoint extended with `rag_publisher` (names our corpus), `sources_limit`, `include_citations`, and `tradition`; retrieval and generation happen in a single call, and the response returns a `citations` array (`item_title`, `publisher`, `snippets`) plus a `sources_returned` boolean. There is no metadata filtering and no relevance scores, so scoping discipline lives in the data: every ingested document is pre-split to one document per book+chapter (per Psalm for the Psalms corpus) and titled by a strict convention ("<Source> — <Book> <Chapter>", e.g. "Psalms Commentary — Psalm 23"), and the Context Agent verifies retrieved citations' `item_title` against the current passage's book before use — a client-side guard against cross-book contamination. The `sources_returned` flag replaces any hand-built "is this a Psalm" gate: the Context Agent always attempts grounding and simply reports "no grounding" when retrieval finds nothing relevant. Corpus growth beyond the Psalms PDF (public-domain whole-Bible commentaries via Data Engine file upload or web scrape) requires **zero code change** — same publisher, same call — and is scheduled as optional data ops after Step 33 ships, with the Psalms-only corpus always remaining a sufficient fallback. Ingestion itself (upload, scrape jobs, metadata editing) happens in Gloo AI Studio and is budgeted as ops work, not repo code.
...
- **What gets built:** A processing script that takes the provided Psalms commentary PDF and splits it into **one document per Psalm**, each titled by the corpus naming convention ("<Source> — Psalm <n>") with source and licence noted in the document metadata (title/tags/summary — the fields the Data Engine exposes). The documents are ingested into the app's Gloo Content Library under a single publisher (the `rag_publisher` value) via Data Engine file upload. The Step 4 `groundedCompletion()` placeholder in `src/lib/gloo.ts` is completed with the verified signature: standard completion params plus `rag_publisher`, `sources_limit`, `include_citations`, `tradition`, and typed response fields `citations[]` (`item_title`, `publisher`, `snippets`) and `sources_returned`. A dev-only route issues a grounded completion for a given passage reference and returns the completion plus its citations.
...
- **How to test it:** Run the Facilitator sweep for a circle on a Psalms plan day: the digest includes commentary-informed context in modern language and shows the attribution line naming the source from the citations; `agent_logs` shows the Context grounded call before the Facilitator run. Run it for a circle reading Mark: `sources_returned` is false (or citations fail the book guard), Context returns its no-op, and the digest has no attribution line. Force the cross-book case in dev (query a Psalm-adjacent theme from a Mark passage): the citation title guard rejects the chunks and the digest stays ungrounded.
- **Definition of done:** Passages with corpus coverage produce RAG-grounded digests with citation-derived source attribution and modernised commentary language; uncovered passages produce untouched digests via the stub's no-op path, with the cross-book guard verified.
