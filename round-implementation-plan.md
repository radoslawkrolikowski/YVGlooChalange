...
- **UI:** Onboarding runs inside the 8A layout shell with a stepped progress indicator (added to the library, reused by Steps 10–12). Language and version options render as tappable selection cards with a clear selected state; the curated short-list appears instantly and skeleton rows fill while the full catalogue loads. The settings screen matches the profile screen's visual language.
