...
//
// The Reminder Agent's output surface: the bell + list in the header. Two
// notification types, both generated by Gloo on the daily Reminder sweep
...
// activity-based proxy in reminders.ts). Delivery is IN-APP ONLY, no email
// (Step 29 as amended). Path A only — Instant Access has no users row and so
// no notification is ever persisted for it (brief §7).
//
...
// daily key ("2026-07-24"), the same shape agent_runs stamps.
//
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    /** "reading" (2+ days behind plan) or "messages" (unread circle messages). */
