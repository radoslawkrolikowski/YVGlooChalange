...
// the plan day's passage reference. Step 30A opens this path to anonymous
// Instant Access sessions: such a row carries `anonSessionId` instead of a
// `userId` (an anon visitor has no users row, brief §7), is scoped to that
// browser session, and is pruned on the Step 31 rail — the same treatment
// Step 30 gave anonymous reflections and messages. A highlight is anonymous
// iff userId is null. The OAuth "imported" source stays Path A only: it needs
// a YouVersion account by definition.
//
...
export const highlights = pgTable(
  "highlights",
  {
    ...
    /** The owner (Path A). Null on Step 30A anonymous in-app highlights, which
     * carry `anonSessionId` instead. Exactly one of the two is ever set. */
    userId: text("user_id").references(() => users.id, {
      onDelete: "cascade",
    }),
    /** Step 30A: the anonymous session that made this highlight — the scoping
     * key for reads and the pruning key (Step 31). Null on Path A rows. */
  ...
  (table) => [
    ...
    // Step 30A: the anonymous read path ("this session's highlights") and the
    // Step 31 prune both key off the session id.
    index("highlights_anon_session_idx").on(table.anonSessionId),
  ],
);
...
// and saved here when they keep it. Step 30A adds the anonymous path: an
// Instant Access visitor's saved prayer is a row carrying `anonSessionId`
// instead of a `userId` (no users row is ever created, brief §7), readable
// only within that browser session and pruned on the Step 31 rail. A prayer is
// anonymous iff userId is null.
//
...
export const savedPrayers = pgTable(
  "saved_prayers",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    /** The owner (Path A); null on Step 30A anonymous prayers. */
    userId: text("user_id").references(() => users.id, {
      onDelete: "cascade",
    }),
    /** Step 30A: the anonymous session that saved it — scoping and prune key. */
    anonSessionId: text("anon_session_id"),
  ...
  (table) => [
    ...
    // Step 30A: the same list for an anonymous session, and the prune key.
    index("saved_prayers_anon_created_idx").on(
      table.anonSessionId,
      table.createdAt,
    ),
  ],
...
// notification types. Two are generated by Gloo on the daily Reminder sweep
// (src/lib/reminders.ts): "reading" (the user is 2+ days behind their active
// plan) and "messages" (unread circle messages older than 24h — the
// activity-based proxy in reminders.ts). The third, "reply" (Step 30A), is
// event-driven rather than scheduled: the public circle's bot member answering
// someone's post notifies that poster. Delivery is IN-APP ONLY, no email
// (Step 29 as amended).
//
// Step 30A relaxes Step 29's anonymous exclusion to SESSION-SCOPED: an Instant
// Access session's notification is a row carrying `anonSessionId` instead of a
// `userId`, so the bell works during the demo while nothing is persisted past
// the session (pruned on the Step 31 rail). No users row is ever created. A
// notification is anonymous iff userId is null. The scheduled Reminder sweep
// stays Path A only — it keys off users.id and plan progress rows.
//
// The unique index on (user, type, period_key) IS the second idempotency fence
// behind the sweep's agent_runs claim ("idempotent per user+type+day"): a
// re-run inserts with onConflictDoNothing and delivers nothing new, so the bell
// can never show two reading reminders for the same day. period_key is the
// daily key ("2026-07-24"), the same shape agent_runs stamps; the event-driven
// "reply" type stamps "reply:<message id>" instead, so the same bot reply can
// never notify twice. Anonymous rows get their own partial unique index on
// (anon_session_id, type, period_key) — a NULL user_id would make every row
// distinct to Postgres and defeat the fence.
//
// The message body is generated DIRECTLY in the user's language (brief §5.12,
// native generation — a reminder is 1:1 with one user, so there is only ever
// one language to write and nothing is translated after the fact).
export const notifications = pgTable(
  "notifications",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    /** The recipient (Path A); null on Step 30A anonymous notifications. */
    userId: text("user_id").references(() => users.id, {
      onDelete: "cascade",
    }),
    /** Step 30A: the anonymous recipient's session — scoping and prune key. */
    anonSessionId: text("anon_session_id"),
    /** "reading" (2+ days behind plan), "messages" (unread circle messages),
     * or "reply" (Step 30A — the circle bot answered this reader's post). */
    type: text("type").notNull(),
    /** Short heading rendered on the card row, e.g. "A passage is waiting". */
    title: text("title").notNull(),
    /** The Gloo-generated pastoral message, in the user's language. */
    body: text("body").notNull(),
    /** Reading reminders: the missed passage's USFM reference; null otherwise. */
    reference: text("reference"),
    /** Human-readable label for the card, e.g. "Psalm 24" or the circle name. */
    label: text("label"),
    /** The circle this notification is about; null when the user is in none. */
    circleId: text("circle_id").references(() => circles.id, {
      onDelete: "cascade",
    }),
    /** The daily period this covers — the idempotency key with (user, type). */
    periodKey: text("period_key").notNull(),
    /** ISO 639-1 language it was generated in — native, never translated. */
    language: text("language").notNull(),
    /** Model that served the generation, as reported by Gloo. */
    model: text("model"),
    /** When the user marked it read (opened the bell); null while unread. */
  ...
  (table) => [
    // Idempotent per user+type+day — the second fence behind agent_runs.
    uniqueIndex("notifications_user_type_period_idx").on(
      table.userId,
      table.type,
      table.periodKey,
    ),
    // The same fence for anonymous rows (Step 30A). Partial, because a NULL
    // user_id in the index above compares as distinct and fences nothing.
    uniqueIndex("notifications_anon_type_period_idx")
      .on(table.anonSessionId, table.type, table.periodKey)
      .where(sql`${table.anonSessionId} is not null`),
    // The bell lists a user's notifications, newest first.
    index("notifications_user_created_idx").on(table.userId, table.createdAt),
    // The same list for an anonymous session, and the prune key.
    index("notifications_anon_created_idx").on(
      table.anonSessionId,
      table.createdAt,
    ),
  ],
