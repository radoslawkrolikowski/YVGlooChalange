...
/**
 * The lesson summary (Step 25): a 3–5 sentence plain-language summary of the
 * passage's main teaching, generated by the Summary agent alongside the digest.
 * Available, never forced — rendered as a disclosure that is COLLAPSED by
 * default, using the same expand pattern as Step 15's pre-reading card
 * (aria-expanded button + rotating chevron) so disclosures feel consistent
 * app-wide. Sits inside the digest card, separated by a sage hairline.
 */
function LessonSummary({ summary }: { summary: string }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="border-t border-sage/40 pt-3">
      <button
        type="button"
      ...
      {open && (
        <p className="mt-2.5 whitespace-pre-wrap break-words font-serif text-[0.98rem] leading-relaxed text-ink">
          {summary}
        </p>
      )}
    </div>
